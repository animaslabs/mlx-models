...
def download_model(repo_id: str, local_dir: str | None = None, revision: str | None = None) -> str:
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Hugging Face model utilities"
    )
    subparsers = parser.add_subparsers(dest="command", required=True)
    
    # Download command
    dl_parser = subparsers.add_parser("download", help="Download a model from HF Hub")
    dl_parser.add_argument("repo_id", help="Repository ID (e.g., nvidia/parakeet-tdt-0.6b-v3)")
    dl_parser.add_argument("--local-dir", "-d", help="Local directory to save the model")
    dl_parser.add_argument("--revision", "-r", help="Git revision (branch, tag, or commit)")
    ...
    up_parser.add_argument("repo_id", help="Target repository ID (e.g., username/model-name)")
    up_parser.add_argument("--private", "-p", action="store_true", help="Create a private repo")
    up_parser.add_argument("--message", "-m", help="Commit message")
    
    # Unpack command
    unpack_parser = subparsers.add_parser("unpack", help="Unpack a NeMo file (tar archive)")
